...
public:
    ...
    void travLevel(VST &visit) { if (_root)_root->traLevel(visit); }
...
template<typename T>
BinNodePos(T)BinTree<T>::insertAsLC(BinNode<T> *x, T const &e) {
    _size++;
    x->insertAsLC(e);
    updateHeightAbove(x);
    return x->lc;
...
template<typename T>
BinNodePos(T)BinTree<T>::insertAsRC(BinNode<T> *x, T const &e) {
    _size++;
    x->insertAsRC(e);
    updateHeightAbove(x);
    return x->rc;
...
template<typename T>
BinNodePos(T)BinTree<T>::attachAsLC(BinNodePos(T)x, BinTree<T> *&s) {
...
template<typename T>
BinNodePos(T)BinTree<T>::attachAsRC(BinNodePos(T)x, BinTree<T> *&s) {
    if (x->rc = s->_root)
        x->rc->parent = x;
    _size += s->_size;
    updateHeightAbove(x);
...
BinTree<T> *BinTree<T>::secede(BinNode<T> *x) {
    FromParentTo(*x) = nullptr;  //定位x并删除
    updateHeightAbove(x->parent);   //更新原树所有父节点的高度
    BinTree<T> *s = new BinTree<T>;    //构造一颗新树
    s->_root = x;
    x->parent = nullptr;             //新树构造完毕，以x为根
    s->_size = x->size();
    _size -= s->_size;
    return s;
}